...
def home():
    ...
    for transaction in transactions:
        ...
        db.session.query(Asset).filter(Asset.id == transaction.id).update(
            {"total_spent": total_spent}, synchronize_session=False
        )
        db.session.commit()
        ...
        portfolio_data = (
            portfolio_data.set_index("date")
            .add(data.set_index("date"), fill_value=0)
            .reset_index()
        )
    ...
    if len(Ticker_List) > 0:
        for ticker in Ticker_List:
            quantity_query = db.session.query(func.sum(Asset.quantity)).filter(
                Asset.ticker == ticker
            )
            quantity_query = [value for value, in quantity_query]
            Ticker_Dict[ticker].append(quantity_query[0])
            totalSpent_query = db.session.query(func.sum(Asset.total_spent)).filter(
                Asset.ticker == ticker
            )
        ...
        Ticker_df["averageCost"] = Ticker_df["totalSpent"] / Ticker_df["totalQuantity"]
        Ticker_df["Color"] = np.where(
            Ticker_df["totalProfit"] < 0, "#ff2800", "#0FFF50"
        )
        pie_fig = go.Figure(
            data=[
                go.Pie(
                    labels=Ticker_df.index,
                    values=Ticker_df["totalValue"],
        ...
        piechart_div_string = pyo.offline.plot(
            pie_fig, include_plotlyjs=True, output_type="div"
        )
        pie_chart = Markup(piechart_div_string)
        bar_fig = go.Figure(
            go.Bar(
                x=Ticker_df.index,
                y=Ticker_df["totalProfit"],
                marker_color=Ticker_df["Color"],
            )
        )
        ...
        bargraph_div_string = pyo.offline.plot(
    ...
    bool_data = False
    return render_template(
        "index.html", bool=bool_data, user=current_user, tickers=Ticker_List
    )
